...
{
    public class TenonSpliceJointX : JointX
    {
        ...
        public TenonSpliceJointX(JointX parent)
        {
        ...
        public override int Construct(Dictionary<int, Beam> beams)
        {
            ...
            var points = new Point3d[]
            {
                ...
                SplicePlane.PointAt(    -tenonWidth / 6,            height * 0.5 + AddedUp, 0),
                SplicePlane.PointAt(    tenonWidth / 6,             height * 0.5 + AddedUp, 0),
                SplicePlane.PointAt(    width * 0.5 + Added,        height * 0.5 + AddedUp, 0),
                SplicePlane.PointAt(    -width * 0.5 - Added,       -height * 0.5 - AddedUp, 0),
                SplicePlane.PointAt(    -tenonWidth / 6,            -height * 0.5 - AddedUp, 0),
            ...
            var topProfile = new Polyline()
            {
                ...
                points[1].Transformed(proj0),
                points[1].Transformed(proj1),
                points[2].Transformed(proj1),
                points[2].Transformed(proj0),
                points[3].Transformed(proj0)
            };

            var bottomProfile = new Polyline()
            {
                points[4].Transformed(proj0),
                points[5].Transformed(proj0),
                points[5].Transformed(proj1),
                points[6].Transformed(proj1),
                points[6].Transformed(proj0),
                points[7].Transformed( proj0)
            };
